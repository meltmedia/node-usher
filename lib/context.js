...
Context.prototype.scheduleActivity = function scheduleActivity(task) {
  ...
  _.each(scheduledActivityOptions, function(value, key) {
    if (_.isFunction(value)) {
      scheduledActivityOptions[key] = value.call(undefined, input);
    }
  });
...
Context.prototype.scheduleChildWorkflow = function scheduleChildWorkflow(task) {
  ...
  // Support options being a function with the signature 'function (input) { return dynamicOptionValue; }'
  _.each(scheduledChildWorkflowOptions, function(value, key) {
    if (_.isFunction(value)) {
      scheduledChildWorkflowOptions[key] = value.call(undefined, input);
    }
  });
