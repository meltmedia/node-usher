  ...
  function evaluateState() {
    ...
    for (var i = 0; i <= state.currentIndex; i++) {
      ...
      var currentExecution = {
        execution: new WorkflowExecution(self.fragment.tasks()),
        context: new Context(ctx.decisionTask, self.name + '-' + i, ctx.localVariables)
      };

      currentExecution.context.setWorkflowInput(input);
    ...
    async.map(executionContexts, function (item, next) {
        ...
        if (item.context.done()) {
          ...
          if (!_.isArray(results) || results.length < 1) {
            return next(null, {
              ...
              results: results
            }); // The results of this execution did resolve the accumulators 'done' state
          } else {
            return next(null, {
              outstanding: false,
              done: false,
              results: results
            }); // The results of this execution did NOT resolve the accumulators 'done' state
          }
        }
