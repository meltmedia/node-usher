...
Context.prototype.didChildWorkflowFail = function didChildWorkflowFail(task) {
  ...
  var control = this.resolvedTaskName(task),
      events = this.eventList._events;

  return events.some(function (event) {
    var initiatedEventId, initiatedEvent;
    if (event.eventType === 'StartChildWorkflowExecutionFailed') {
      initiatedEventId = event.startChildWorkflowExecutionFailedEventAttributes.initiatedEventId;
    ...
    } else if (event.eventType === 'ChildWorkflowExecutionTimedOut') {
      initiatedEventId = event.childWorkflowExecutionTimedOutEventAttributes.initiatedEventId;
      initiatedEvent = this.eventList.eventById(initiatedEventId);
      if (initiatedEvent.startChildWorkflowExecutionInitiatedEventAttributes.control === control) {
        return true;
      }
    } else if (event.eventType === 'ChildWorkflowExecutionFailed') {
      initiatedEventId = event.childWorkflowExecutionFailedEventAttributes.initiatedEventId;
      initiatedEvent = this.eventList.eventById(initiatedEventId);
      if (initiatedEvent.startChildWorkflowExecutionInitiatedEventAttributes.control === control) {
        return true;
      }
