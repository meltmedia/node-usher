 ...
 * @param {string} [version] - Version of the workflow to execute (conforms to v2.0 of http://semver.org) - default: 1.0.0
 * @param {Function} [cb] - Callback when done
 * @returns {Workflow} This workflow version so you can configure the actual decider.
 */
DecisionPoller.prototype.execute = function execute(input, version, cb) {

  if (!cb && !input && !version) {
    throw new Error('No paramaters provided for workflow execution');
  }

  if (!cb) {
    if (_.isFunction(version)) {
      // No version defined, shift left
      cb = version;
      version = '1.0.0'; // Match first
    } else if (!version) {
      // No version and no callback provided
      version = '1.0.0'; // Match first
      cb = function () {}; // NoOp
    } else {
      // No callback defined
